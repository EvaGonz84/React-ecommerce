...
const NavLink = ({ textHome, textProduct, textContact, closeMenu }) => {
  return (
    <>
      ...
      <Link onClick={closeMenu} to="/" className="relative py-6">
      ...
      <Link onClick={closeMenu} to="/product" className="relative py-6">
        <span className="peer hover:text-yellow-600">{textProduct}</span>
        <span className="absolute bottom-5 left-0 block h-1 w-full scale-x-0 transition-all duration-300 peer-hover:scale-x-100 peer-hover:bg-yellow-600"></span>
      </Link>

      <Link onClick={closeMenu} to="/contact" className="relative py-6">
